...
def encode_images(image,name):
    if os.path.isfile(os.path.join(directory,'../dump/face_names')):
        file = open(os.path.join(directory,"../dump/face_names"),'rb')
        known_face_names = pickle.load(file)
        file.close()
        file = open(os.path.join(directory,'../dump/face_encoding'),'rb')
    ...
    if image is not None:
        ...
        if len(enc) !=0:
            ...
            file = open(os.path.join(directory,"../dump/face_names"),'wb')
            pickle.dump(known_face_names,file)
            file.close()
            file = open(os.path.join(directory,"../dump/face_encoding"),'wb')
...
def label_image(request):
    ...
    return {"status":True,"message":"Face Has Been Recognized","label":face_names}       